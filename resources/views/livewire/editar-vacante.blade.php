<form class="w-1/2 space-y-5" wire:submit.prevent='editar_vacante'>
     ...
     <div class="mt-4">
      ...
      <!--seccion para desplegar un preview de la imagen seleccionada-->
       <div>
        ...
        <!--el preview imagen que se muestra desde la base de datos -->
          <!--seccion para desplegar un preview de la imagen seleccionada-->
          <div class="my-5 w-64">
            <!--evaluando si la variable de imagen tiene contenido-->
             ...
             <x-label for="imagen" :value="__('Imagen Actual')" />
                 ...
                 <img src="{{asset('storage/vacantes/' . $imagen)}}" class="w-64" alt="{{'Imagen vacante '. $titulo}}"/>
             @endif
 
          </div>
        
          <!--el preview imagen que se muestra desde el input -->
          
          <!--seccion para desplegar un preview de la imagen seleccionada-->
          <div class="my-5 w-64">
            <!--evaluando si la variable de imagen tiene contenido-->
             @if ($imagen_nueva)
             <x-label for="imagen_nueva" :value="__('Imagen Nueva')" />
                 <!--Imagen-temporaryUrl crea una url temporal para mostrar la imagen como preview-->
                 <img src="{{$imagen_nueva->temporaryUrl()}}" class="w-64" alt="{{'Imagen vacante '. $titulo}}"/>
             @endif
