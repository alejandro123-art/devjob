<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Notificaciones') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h1 class="text-3xl mb-10 font-bold text-center">Mis Notificaciones</h1>
                 <div class="divide-y divide-gray-200">
                   <!--Mostrando las notificaciones no leidas -->
                   @forelse ($notificaciones as $notificacion)
                    <div class="p-5 border boder-gray-200 lg:flex lg:justify-between lg:items-center">
                         
                        <div>
                                 <!--Acediendo dentro de el campo data en el registro nombre de vacante dentro del arreglo-->
                                  <p >Tienes un Nuevo candidato en: <span class="font-bold">{{$notificacion->data['nombre_vacante']}}</span></p>

                                 <!--tiempo de creada la vacante-->
                                  <p ><span class="font-bold">{{$notificacion->created_at->diffForHumans()}}</span></p>

                         </div>

                         <div class="mt-5 lg:mt-0">
                               <a href="{{route('candidatos.index',$notificacion->data['id_vacante'])}}" class="bg-indigo-500 p-3 text-sm uppercase font-bold text-white rounded-lg">Ver Candidatos</a>
                         </div>
                       
                    </div>   
                   @empty
                        <p class="text-center text-gray-600">No hay notificaciones nuevas</p>
                   @endforelse
                  
                </div>
              </div>
            </div>
        </div>
    </div>
</x-app-layout>